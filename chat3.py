
import socket
import threading
import json

# Colores para formatear mensajes
wht = '\033[00m'
red = '\033[31m'
grn = '\033[32m'
ylw = '\033[33m'

# Datos del servidor
ip = '127.0.0.1'
puerto = 8685
max_conexiones = 5
servidor = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
servidor.bind((ip, puerto))
servidor.listen(max_conexiones)

users = []  # Lista de usuarios conectados

def broadcast_message(message):
    """Envía un mensaje a todos los usuarios conectados."""
    json_message = json.dumps(message)
    for user in users:
        try:
            user[0].send(json_message.encode('utf-8'))
        except:
            user[0].close()
            remove_user(user)  # Remover usuarios que no están disponibles

def remove_user(user):
    """Elimina a un usuario de la lista cuando se desconecta."""
    if user in users:
        users.remove(user)
        print(f"[*] Usuario {user[1]} desconectado y eliminado.")

def conexiones(socket_cliente, username):
    """Maneja la conexión de un cliente y recibe mensajes."""
    try:
        while True:
            peticion = socket_cliente.recv(1024).decode('utf-8')
            if not peticion:
                break  # Si no se recibe nada, significa que el cliente se desconectó
            message = json.loads(peticion)
            if message["msg"]:
                print(f'\n{message["user"]}{ylw}:{wht} {message["msg"]}')
                if message["to"] == 'all':
                    broadcast_message(message)
            if message["msg"] == 'exit':
                print(f'[*] {message["user"]} se ha desconectado.')
                break
    except:
        print(f'[!] Error con el usuario {username}')
    finally:
        # Cerramos el socket del cliente y lo eliminamos de la lista
        socket_cliente.close()
        remove_user([socket_cliente, username])

def sendMessage():
    """Permite enviar mensajes desde el servidor a todos los clientes."""
    while True:
        msg = input('Servidor >>> ')
        if msg:
            message = {
                "user": "server",
                "msg": msg,
                "to": "all"
            }
            broadcast_message(message)

def solicitar_username(socket_cliente):
    """Solicita y verifica el nombre de usuario del cliente."""
    while True:
        socket_cliente.send("Por favor, ingresa tu nombre de usuario: ".encode('utf-8'))
        username = socket_cliente.recv(1024).decode('utf-8')
        
        # Verificar si el nombre de usuario ya está en uso
        if not any(user[1] == username for user in users):
            socket_cliente.send(f"Bienvenido, {username}!".encode('utf-8'))
            return username
        else:
            socket_cliente.send("Nombre de usuario ya en uso. Intenta con otro.".encode('utf-8'))

def handler():
    """Maneja la aceptación de nuevas conexiones de clientes."""
    print(f"[*] Esperando conexiones en {ip}:{puerto}")
    while True:
        user_socket, direccion = servidor.accept()
        print(f"[*] Conexión establecida con {direccion[0]}:{direccion[1]}")
        
        # Solicitar nombre de usuario
        username = solicitar_username(user_socket)

        # Agregar el nuevo usuario a la lista
        users.append([user_socket, username])

        # Enviar mensaje de bienvenida a todos los clientes
        message = {
            "user": "server",
            "msg": f"{username} se ha unido al chat.",
            "to": "all"
        }
        broadcast_message(message)

        # Iniciar un hilo para manejar la conexión de cada cliente
        client_thread = threading.Thread(target=conexiones, args=(user_socket, username))
        client_thread.start()

# Ejecutar el servidor
try:
    # Iniciar el hilo para enviar mensajes desde el servidor
    admin_thread = threading.Thread(target=sendMessage)
    admin_thread.start()

    # Manejar conexiones de clientes
    handler()
except KeyboardInterrupt:
    print('\n[*] Cerrando servidor...')
    servidor.close()
